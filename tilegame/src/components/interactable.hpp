...
namespace tilegame::components
{
     ...
     * the Interactable entity, so Lua can scope a listener to one specific NPC via
     * `_add_event_listener(_InteractEvent, callback, npc_entity)`. A plain value type - never
     * stored as an entt component - that only exists for the duration of that call. Exposed to
     * Lua as `_InteractEvent`.
     */
    struct InteractEvent
    {
        ...
        InteractEvent() : player(entt::null) {}
        InteractEvent(entt::entity player) : player(player) {}
        [[nodiscard]] std::string to_string() const;
